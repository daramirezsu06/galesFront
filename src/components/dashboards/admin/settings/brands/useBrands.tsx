...
const useBrands = ({ allBrands }) => {
  ...
  const handleRefresh = (value) => {
    switch (action) {
      case Actions.NEW:
        ...
        break;
      case Actions.EDIT:
        const newValue = brands.map((brand) =>
          brand.id === value.id
            ? { ...value, productCount: brand.productCount }
            : brand
        );
        setBrands(newValue);
        break;
  ...
  const ExpandedComponent = ({ data }) => (
    <div className="p-8 flex flex-col gap-4 bg-gray-300">
      <div className="flex gap-4 justify-end md:hidden">
        <div onClick={() => onDelete(data)} className="btn-icon">
          <CircleButton className="p-2 rounded-full cursor-pointer hover:bg-purple-950/20">
            <TrashIcon className="text-red-700 w-6 h-6" />
          </CircleButton>
        </div>
