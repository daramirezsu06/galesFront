...
const NewEditCategory = ({ category, handleCancel, handleRefresh, action }) => {
  const {
    ...
    handleChange,
    handleSubmit,
  } = useNewEditCategory({ category, handleRefresh, action });
  ...
  return (
    <form
      ...
      <div className="flex justify-evenly items-center w-full">
        <h2 className="text-xl font-bold">Categoría {data.name}</h2>
        {/* image */}
        <div className="relative h-28 w-40 flex items-center justify-center">
          ...
          <label
            htmlFor="input-file"
            className="cursor-pointer absolute top-0 left-0 bottom-0 right-0 w-full h-full bg-black/50 backdrop-blur-sm text-center flex justify-center items-center rounded transition-opacity duration-1000 ease-out opacity-0 hover:opacity-100">
      ...
      <div className="col-span-2 flex justify-between">
        <button onClick={handleCancel} type="button" className="btn btn-cancel">
          Cancelar
        </button>
